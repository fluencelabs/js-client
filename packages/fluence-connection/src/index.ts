...
export class RelayConnection extends FluenceConnection {
    ...
    async sendParticle(
        ...
        */
        ...
        const conn = await this._lib2p2Peer.dialProtocol(
            this._relayAddress,
            PROTOCOL_NAME
        );
    ...
    async connect(onIncomingParticle: ParticleHandler) {
                        ...
                        try {
                            for await (const msg of source) {
                                try {
                                    ...
                                    log.error(
                                        "error on handling a new incoming message: " +
                                            e
                                    );
                                }
                            }
        ...
        log.debug(
            `dialing to the node with client's address: ` +
        ...
        try {
            ...
            if (e.name === "AggregateError" && e._errors?.length === 1) {
                ...
                throw new Error(
                    `Error dialing node ${this._relayAddress}:\n${error.code}\n${error.message}`
                );
            } else {
                throw e;
            }
