...
): fluencePeerCandidate is IFluenceClient => {
...
export const v5_callFunction = async (
    ...
    const {
        ...
        client: peer,
        config,
    } = await extractFunctionArgs(rawFnArgs, def);
 ...
 */
export const v5_registerService = async (
    args: any[],
    def: ServiceDef,
): Promise<unknown> => {
    const { peer, service, serviceId } = await extractServiceArgs(
        args,
